...
Table tramite {
  id                 uuid      [pk]
  expediente_id      uuid      [not null, ref: > expediente.id]
  tipo_tramite_id    uuid      [not null, ref: > tipo_tramite.id]
...
Table expediente {
  id            uuid        [pk]
  entidad_id    uuid        [note: 'ID externo del dominio']
  entidad_ref   varchar(50) [not null, note: 'Referencia lógica: COMERCIO, TCI, COMPRA, etc']
  estado        expediente_estado [not null, default: 'ABIERTO']

  created_at    timestamptz [default: `now()`]
  updated_at    timestamptz [default: `now()`]

  Note: 'Nodo genérico de trazabilidad de trámites'
}
